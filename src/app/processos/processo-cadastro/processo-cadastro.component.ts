import { ProcessoFiltro } from '../processo.service';
...
import { AndamentoService, AndamentoFiltro} from '../../andamentos/andamento.service';
...
export class ProcessoCadastroComponent implements OnInit {
 ...
 //       this.comarcaSelecionada = (processo.localizacao.comarca) ?
  //      processo.localizacao.comarca.codigo : null;

 //       this.competenciaSelecionada = (processo.localizacao.competencia) ?
 //       processo.localizacao.competencia.codigo : null;
  ...
  carregarLocalizacao() {
    ...
    this.processoService.listarLocalizacoes().then(localizacao => {
      this.localizacoes = localizacao.map(p => ({ label: p.comarca.nome, value: p.codigo }));
    })
    .catch(erro => this.errorHandler.handle(erro));
    }


    carregarLocalizacao_antiga() {
